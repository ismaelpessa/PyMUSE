...
Visualization
=============
In any moment, you can clean the primary canvas by usin/
    cube.reload_canvas(/
...
where [10,5,35] corresponds to the semimajor axis, semiminor axis and rotation angle respectivel/
...
Another extra feature is given by the  functioni::
...
Get the spectrum of a region defined in a DS9 .reg fil/
=======================================================/
...
As you have noted, all the diferent `get_spec_` functions have the keyword argument "mode". The mode availables to combine the spectrum of the diferent spaxels in a region ar/
              ...
              * `sum` - Sum of total flu/
              * `gaussian` - Weighted mean. Weights are obtained from a 2D gaussian fit of the bright profil/
              * `wwm` - 'White Weighted Mean'. Weigted mean, weights are obtained from the white image, smoothed using a gaussian filter of sigma = npix. If npix=0, no smooth is don/
              * `ivarwv` - Weighted mean, the weight of every pixel is given by the inverse of it's varianc/
              * `mean`  -  Mean of the total flu/
              * `median` - Median of the total flu/
              * `wwm_ivarwv` - Weights given by both, `ivarwv` and `wwm/
              * `wwm_ivar` - Weghts given by both, `wwm` and `ivar/
              * `wfrac` - It only takes the fraction `frac` of brightest spaxels (white) in the regio/
...
Some modes of extraction require a npix value (default = 0). This value correspond to the sigma of the gaussian functio/
...
If `empirical_std = True` (default = False) the uncertainties of the spectrum will be calculated empiricall/

`n_figure` is the number of the figure that will display the new_spectru/

if `save` = True (default = False) The new spectrum extracted will be saved to the hard dis/
...
Option, you saved the spectrum to the hard-disk as a fits file. To access the data you can use::
    ...
    from linetools.spectra.io import readspe/
    spectrum = readspec('spectrum_fitsname'/

This will create a ``XSpectrum1D`` object from the fits file. You can access to the spectrum wavelength, flux and sigm/
typing spectrum.wavelength, spectrum.flux and spectrum.sig. Additional information on the ``XSpectrum1D`` Class can be found in https//github.com/linetools/linetools/blob/master/linetools/spectra/xspectrum1d.p/



Use a SExtractor output file as an inpu
========================================
...
Where sextractor_filename is the name of the SExtractor's output. Every source with a SExtractor flag highe/
...
This will save in the hard disk the spectra of all the sources defined in the sextractor_filename which flags be lower or equal than flag_threshold using the specified mode/
...
image given to SExtractor will be probably different to the conversion in the MUSE cube.  You may want to include the parameter/
    * X_WORLD/
    * Y_WORL/
    * A_WORL/
    * B_WORL/
...
Save a set of spectra defined by a MUSELET output fits tabl/
...
Will allow  you to estimate the seeing using the white image. The user must insert as the input the xy coordinates in spaxel spac/
...
Image creation
==============
Create image collapsing the Cub/
...
To do that, you may want to use the metho/
...
Create a smoothed white imag/
=============================/
The method::

    cube.get_smoothed_white(npix=2, save=True, kwargs/

returns a smoothed version of the white image. ``npix`` defines the sigma of the gaussian filter.  kwargs are passed t/
...
Compose a filtered imag/
========================/

If you want to do a photometric analysis from the Muse Cube, you would need to convolve your data with a photometric filte/
and compose a new filtered image. To do this, you can use the method::

    cube.get_filtered_image(_filter = 'r')

This method will write a new filtered image that will be useful to photometry analysi/
...
estimates de kinematics of the elliptical region defined by (x_c,y_c,params) in spaxels. The method extract the 1-D spectrum of every spaxel withi/
the region and fit a gaussian + linear model, in order to fit and emi/abs line and the continuum. The required paramters ar/
    * x_/
    * y_/
    * param/
That define the elliptical region/
    * wv_line_vac wavelength of the transition in vacuum/
    * wv_range_size Angstroms. Space at each side of the line in the spectrum. Set this parameter in order to fit the complete transition but do not include near additional line/
...
Will create a video which frames will be, at each redshifts, the sum of all wavelengths that would fall at strong emission line/
(Ha,Hb,OII,OIII)::
